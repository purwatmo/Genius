...
unsigned char	EEMEM __txt_rcpt[8][39] = {{"PT. Hanindo Automation  Solutions"},
...
//	while(1); 
	if ((rcv_trans[35]==0x39) & (rcv_trans[36]==0x39)) {
		c2=0;
		for(c1=37;c1<44;c1++) {_msg_99_trans[c2]=rcv_trans[c1];c2++;}
...
unsigned char _menu_idle(void){
	unsigned char	__key, __num, __cnt = 0;//, __cbb = 0;
//	char	__i, __x;
//	char	__addr[2] = {"@\0"};
	char	__date[9];
	char	__time[9];

	char	f_ot=0,f_an=0,cntf=0;
	...
	while(1){
		...
		if (f_rcv==1) {f_rcv=0;struc_dat();
			if (struc_dat()==0x10) {
				_lcd_printf(1, 1, PSTR("Tidak Ada Transaksi  "));__cnt=0;f_ot=1;}
			if (struc_dat()==0x01) {
			if (f_odo==0) _menu_print("","",1);
			if (f_odo==1) {_menu_print(__nopol, __odo,1);f_odo=0;}
			_scr_idle();}//if struc_dat
		}//if f_rcv
//        if (__num=='*') _menu_print("","",1);//short cut ngeprint
...
//	_lcd(__chr);
	char_count++;
	if (__chr==0x02) {f_rcv=1; char_count=0;}
	if (__chr==0x02) char_count=0;
