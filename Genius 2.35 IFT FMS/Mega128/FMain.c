...
char IsBusyMsg11=False;

char strReceiptNum[7];
char strProduct[15],strBalanceCode[2];
...
void FMenuIdle(){
    ...
    case miRunTotalizer://Standalone
		 ...
		 stMenuIdle=miReady;	      
	     break;
    case miRunTestChar://Slave,Standalone
...
ISR(USART1_RX_vect){
	...
	if (dataTX1==0x02){
        ...
        //   _uart(0,1,rcv_trans[i]);
       // 	}
//        		sprintf_P(serialSend,PSTR("Length:%d "),transLength);
  //      		_uart_print(0,1,serialSend);
...
static char iPos=0,iWait=0,KeyChar,KeyCharLast=' ',BalanceType=0,strFIP[3];//,FIPResult;
static char KeyPressed=0,iValuePos=0,iNozzle=0;
static char ValueChar[10],ProductID[6],FIP[8];       
static char ProductName[11],FIP_Used=0,ProdId=0,IsFullAuthorized=False;
static unsigned int iLoop=0; 
       char lcdteks[20],uiResult;
	   ...
	   Result=MENU_NONE;
           ...
           case laSelectFIPInput:
				...
				if (((KeyChar>='1')&&(KeyChar<='8'))||
					...
					if (FIP_Used<=99){
						...
						//FIP-->ProductID
						//Get ProductID of Selected FIP 
						//ProductName[ProductID[Nozle]] DefProductName[ProductID][ProductName] 
						//if (KeyPressed!=_KEY_ENTER) KeyCharLast=KeyChar;
           ...
           case laOdometerInput:
			    ...
			    KeyPressed=_key_scan(1);
			    KeyChar=_key_btn(KeyPressed);
				if ((KeyChar>='0')&&(KeyChar<='9')&&(iValuePos<7)){//Max 1234567
				    ValueChar[iValuePos]=KeyChar;
					iValuePos++;
					lcd_xy(3,(9+iValuePos));_lcd(ValueChar[iValuePos-1]); 
					_delay_ms(200);
					}
			    if ((iLoop%2000)==0){
				   lcd_xy(3,(10+iValuePos));_lcd('_');
			    }
			    if ((iLoop%2000)==1000){
				   lcd_xy(3,(10+iValuePos));_lcd(' ');
			    }
				if (KeyPressed==_KEY_CANCEL){
				    if (IsFullAuthorized==True)stLocalAccount=laSelectBalanceType;
					else stLocalAccount=laBalanceValue;		        
					}
				else
				if (KeyPressed==_KEY_ENTER){				    
                    ValueChar[iValuePos]=0;
					sprintf_P(strOdometer,PSTR("%s"),ValueChar);
				    stLocalAccount=laDataConfirm;
					}
                */
...
static char KeyChar;//,iWait=0,iPos=0;
...
static char stSettingHeader=shInitHeader,HeaderIdx=0,KeyChar;
...
static char i,Result,keyChar,x,y,iInput,keyPressed=0,IP_blok[4];
     char lcdteks[20];	 
...
static char stMenuProduct=mpInitProduct,ProdID,uiResult=USER_NONE;//,IsMenuProductExit=False;
...
//	char lcdteks[20];
	...
	for(__i = 0; __i < 6; __i++){
		...
		else sprintf_P(__buff,PSTR("%d"),__value[__i]);
	...
	while(1){	
		...
		if(__num >= '1' && __num <= '6'){
			...
			if((__num == 0x35)||(__num == 0x36))
			    sprintf_P(__buff,PSTR("%d"),__value[__num-'1']);
            else sprintf_P(__buff,PSTR(".2%d"),__value[__num-'1']);
...
char procMessage57(){
	 ...
	 //Message57
		 ...
		 /*
		 _uart_printf(0,1,PSTR("procMessage57()"));

         sprintf_P(serialSend,PSTR("Result:%d"),Result);
		 _uart_print(0,1,serialSend);
		 
		 _uart_print(0,1,strCardID);
		 _uart_print(0,1,strCardHolder);
		 _uart_print(0,1,strBalanceCode);
		 _uart_print(0,1,strBalanceTypePrint);
		 _uart_print(0,1,strBalance);
		 _uart_print(0,1,strLicPlate);
...
void PaperCut(){
     ...
     _uart(COM_PRINTER, 1, 0x1B);
	 if(CutType== 1) { _uart(COM_PRINTER, 1, 0x6D);}
	 if(CutType== 2) {_uart(COM_PRINTER, 1, 0x69);}
...
char PrinterType;
	 ...
	 if (PrinterType==PT_CUSTOM_CUBE){
	     _uart(COM_PRINTER, 1, 0x1D);_uart(COM_PRINTER, 1, 0x21);_uart(COM_PRINTER, 1, 0x01);
		 }
}
...
char PrinterType;
     PrinterType=eeprom_read_byte(&DefPrinterType);
	 if (PrinterType==PT_CUSTOM_CUBE){
	     _uart(COM_PRINTER, 1, 0x1D);_uart(COM_PRINTER, 1, 0x21);_uart(COM_PRINTER, 1, 0x00);
		 }
}
...
char PrinterType;
	...
	if (PrinterType==PT_CUSTOM_TG02){
		_uart(COM_PRINTER, 1, 0x1D);_uart(COM_PRINTER, 1, 0x7C);_uart(COM_PRINTER, 1, 0x00);
		//Double Strike ON : 1B 47 01
		_uart(COM_PRINTER, 1, 0x1B);_uart(COM_PRINTER, 1, 0x47);_uart(COM_PRINTER, 1, 0x01);
	...
	if (PrinterType==PT_CUSTOM_CUBE){
        //Print Density 0%
		_uart(COM_PRINTER, 1, 0x1D);_uart(COM_PRINTER, 1, 0x7C);_uart(COM_PRINTER, 1, 0x04);
		//Double Strike OFF : 1B 47 00
		_uart(COM_PRINTER, 1, 0x1B);_uart(COM_PRINTER, 1, 0x47);_uart(COM_PRINTER, 1, 0x00);
		//Font Setting: 1D 21 01
		_uart(COM_PRINTER, 1, 0x1D);_uart(COM_PRINTER, 1, 0x21);_uart(COM_PRINTER, 1, 0x00);   
		}
}
...
static char strPrint[45],iMessage=0,MessageLine=0,IsSignedField=False;
...
static char iPrinted=0,PrintCopy=0;
     ...
     case piCheckPrintStatusHeader:
	      if (IsBusyPrint==False){
		      _uart(COM_PRINTER,1,strPrint[iSend]);
		      iSend++;
		      stPrintIdle=piPrintHeader;
		  }
     ...
     case piInitScroll:
		  ...
		  if ((eeprom_read_byte(&DefPrinterType)==PT_CUSTOM_CUBE)||(iPrinted>=PrintCopy)){
			  ...
			  IsBusyPrint=False;
			  _uart_printf(0,1,PSTR("----Scrolled1----"));
			 }
...
void CarriegeReturn(){
     _uart(COM_PRINTER, 1, 0x0D);     
     _uart(COM_PRINTER, 1, 0x0A);
...
static char zSend;     
	 if (xSend==0x19){
	     if (zSend==xSendLead)_uart(COM_PRINTER,1,xSendLead);
		 else _uart(COM_PRINTER,1,' ');	 
	 } 
	 else _uart(COM_PRINTER,1,xSend);
	 zSend=xSend;
}
...
static unsigned int iSend=0,LSend=0,iLoop=0;
     ...
     case fpPrintHeader:
		  ...
		  if (iSend<40){
		      iLoop++;
			  if ((iLoop%PRINT_DELAY)==0){
			     //_uart(COM_PRINTER,1,strHeader[iSend]);
		         //iSend++;
				 TimPrintBusy=0;
     ...
     case fpCheckPrintStatusMessage:
	      if (IsBusyPrint==False){
              if ((PrintBuffer[iSend]!=0x0D)||(PrintBuffer[iSend]!=0x0A))
			     ...
			     // _uart(COM_PRINTER,1,PrintBuffer[iSend]);
	 ...
	 case fpPrintMargin:
		  if (iMargin<PRINT_MARGIN){
		      iMargin++;
		      iLoop++;
	          if ((iLoop%PRINT_DELAY)==0)_uart(COM_PRINTER,1,' ');
			 }
          else stFreePrinting=fpPrintMessage;
	      break;
     case fpInitFooter:
