...
char IsMenuFIPInit=False;

//Tim System
char TimTicker=0,TimPressed=0;
volatile char TimDisplay=0,TimLocalAccount=0;
...
unsigned char	EEMEM DefPrinterType =PT_CUSTOM_ESCPOS;
...
unsigned char	EEMEM DefPrintLogo = 1;
unsigned char	EEMEM DefPrintSize = 3;
//Logo Position
unsigned int EEMEM DefLogoPosition = 0;
unsigned int EEMEM DefLogoLength = 110 ;
...
unsigned char	EEMEM DefProductName[6][13] ={{"Pert+   "}, 
		 ...
		 char EEMEM DefLocalAccountTimeout=30;
...
void StoreStandaloneTransData(char *strRawTransData){//  [0x50,0x05]  01      01      1       004500   00000200  00000012 [0x06,0x60] = 24
	 ...
	 if (FIPAddr>0){
		 ...
		 if (TransactionStatus=TS_NEW){ 
             ...
             //Genius 4.23 ->moved to GenerateStandaloneTransData
			 //Formating after multiplication
...
/*     char i,FIP,Length;
	 ...
	 FIP=GetFIPAddr(iPumpID);
	 Length=strlen(RecPumpData[FIP].strTransNum);
...
static char zstLocalAccount,MaxLATim=0;
       ...
       #ifdef DEBUG_LA_STATE
	   static char zView;
	   char strSend[20];
	   if(zView!=stLocalAccount){
	      zView=stLocalAccount;
		  sprintf_P(strSend,PSTR("LA:%d"),zView);
		  uart_print(0,1,strSend);	      	   
	   }
	   #endif
...
static char iPrinted=0,PrintCopy=0,iPumpID;
       char FIPAddr;
