...
// MMC
unsigned long int ActiveSector=0,IsWrite,IsRead;
...
struct sMMCProperties{
       char IsCardExist;
	   char CardState

       }MMC;

void systemMMC(){//Automatic 
     static char stSystemMMC=mmcIsExist;

	 switch(stSystemMMC){
	 case mmcIsExist:
	      if (IsCardExist==True){
		      if (CardState==CS_INSERTED){
...
//----------------MMC--------------
void FTestMMC(){
static char stTestMMC=tmScanMMCExist;//tmDisplayInit;
...
unsigned int i;
	 ...
	 case tmInitialize:
	      /* 
	      if (mmc_initialize()==MMC_INITIALIZED){
